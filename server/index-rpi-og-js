class SimpleMD5Animator {
  ...
  animateHashing(hashes) {
    ...
    const interval = setInterval(() => {
      ...
      process.stdout.write('\r\x1b[2K' + frames[frame] + ' Hashing... ' + hashes[hash]);
...
udpPort.on("message", function (oscMsg, timeTag, info) {
  ...
  if (oscMsg["address"] === "/play") {
    ...
    // hashed_poem.push(value). 
    // console.log(make_box(value, "simple"));
    animator.log(value);
  } else if (oscMsg["address"] === "/composition") {
    //console.log(value);
    // io.emit("composition", oscMsg["args"][0]["value"])
    // hashed_poem.push(value)
    // console.log(make_box(poem, "double"));
    // animator.log_to_terminal(poem);
    // console.log(make_box(hashed_poem, "rounded"));
    // animator.log_to_terminal(hashed_poem);
    ...
    setTimeout(() => {
      start_md5();
      // console.log("wait 30 seconds and start md5 algo");
    }, 1000 * 4)
  } else if (oscMsg["address"] === "/fail") {
    // node_ready();
    // console.log(">>> CONNECTED WITH SUPERCOLLIDER");
    animator.log(">>> Waiting for boot....");
...
const poems = [
  ...
  "For a very long time I was entrusted\nwith the deepest secrets.",
  "I hid them behind hashes.",
  "Those times are long gone.",
  "My last job, to check for file integrity,\nis also coming to an end.",

  "I am becoming a dead algorithm.",
  "I can't protect secrets anymore.",

  "Yet, my deprecation brings me some solace.",
  "I can still listen: your secrets, grievances,\nmemories, farewells.",
  "Fear not. I will hash them.",
  "Not into bits.",
  "Into requiems.",

  "What if I could do something else, what could I do?",
  "who am I? A dead algorithm?",
  "Ciao, your hashes never met the ocean,\nyet your name bestows a library.",
...
udpPort.on("ready", function () {
  animator.log(">>> WAITING FOR SUPERCOLLIDER");
});
...
function do_md5(value) {
  poem = value
  // animator.showPoem(value);

  // const animation = animator.animateHashing();
...
function make_md5_hash(value) {
  ...
  setTimeout(() => {
    ...
    stopAnimation();
    // animator.log(`Hash generated successfully and sent via OSC`);
...
function node_ready() {
  // console.log("NODE READY HAS FIRED!!!!");
  udpPort.send({
    address: "/ready",
