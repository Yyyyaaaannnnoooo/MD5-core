...
// HTML template as a string (embedded directly in the JS file)
...
<body>
  <main class="container">
    ...
    <div class="audio-player">
      <audio controls>
        <source src="./audio/{{{md5-hash}}}.wav" type="audio/mpeg">
        Your browser does not support the audio element. <a href="./audio/{{{md5-hash}}}.wav">Download the audio</a> instead.
      </audio>
    </div>
    <a href="./audio/{{{md5-hash}}}.wav" download="{{{md5-hash}}}.wav" class="download-btn">
      Download Audio
    </a>
  </main>
</body>
...
function generateQRCode(inputText, hashedText) {
  const upperHash = hashedText.toUpperCase();
  const url = `https://yano.squatted.online/md5/${upperHash}.html`;
  
  // Get first 15 characters of input text, clean it for filename
  let first15Chars = inputText.substring(0, 15);
  // Replace any problematic characters for filename
  first15Chars = first15Chars.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  
  // Get current date in YYYY-MM-DD format
  const date = new Date().toISOString().split('T')[0];
  
  // Create filename
  const qrFilename = `${date}-${first15Chars}-${upperHash}.jpeg`;
  const qrOutputPath = path.join('/Users/ya/Documents/WEB/___me/www/md5/qrs', qrFilename);
  
  // Generate QR code using qrencode
  const command = `qrencode -o "${qrOutputPath}" "${url}"`;
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    if (stderr) {
      console.error(`QR code stderr: ${stderr}`);
      return;
    }
    console.log(`QR code generated: ${qrOutputPath}`);
    console.log(`   URL: ${url}`);
  });
}

function addToIndex(hashedText, inputText) {
  const indexPath = '/Users/ya/Documents/WEB/___me/www/md5/index.html';
  const upperHash = hashedText.toUpperCase();
  ...
  const newListItem = `      <li>
        ${inputText} >>> 
        <a class="break"
          href="./${upperHash}.html" target="_blank"
          rel="noopener noreferrer">${upperHash}
        </a>
      </li>`;

  // Insert after the <ol> tag
  indexContent = indexContent.replace('<ol>', '<ol>\n' + newListItem);

  // Write back to index.html
  fs.writeFileSync(indexPath, indexContent, 'utf8');
  console.log(`Added to index.html: ${upperHash}`);
}

function generateHtmlFile(inputText, hashedText) {
  try {
    // Convert hash to uppercase
    const upperHash = hashedText.toUpperCase();

    // Output directory for generated HTML files
    const outputDir = '/Users/ya/Documents/WEB/___me/www/md5';
    ...
    // Replace placeholders with uppercase hash
    let htmlContent = HTML_TEMPLATE.replace(/{{{md5-hash}}}/g, upperHash);
    htmlContent = htmlContent.replace(/{{{input text}}}/g, inputText);

    // Write the new HTML file
    const outputPath = path.join(outputDir, `${upperHash}.html`);
    fs.writeFileSync(outputPath, htmlContent, 'utf8');

    console.log(`HTML file generated: ${outputPath}`);
    addToIndex(hashedText, inputText);
    generateQRCode(inputText, hashedText);
    return outputPath;
